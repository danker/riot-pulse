...
""".strip()
    ...
    def test_provider(
        ...
        try:
            # Get provider
            provider = get_llm_provider(
                provider_override=provider_name, model_override=model
            )
    ...
    def benchmark_providers(
        self, providers: list[str] | None = None, custom_prompt: str | None = None
    ) -> BenchmarkResult:
        """
...
def main():
    ...
    benchmark_parser = subparsers.add_parser(
        "benchmark", help="Benchmark all providers"
    )
    benchmark_parser.add_argument(
        "--providers", nargs="*", help="Specific providers to test"
    )
    ...
    if args.command == "dry-run":
        ...
        result = tester.test_provider(
            provider_name=args.provider, model=args.model, custom_prompt=args.prompt
        )
        if result.success:
        ...
        result = tester.benchmark_providers(
            providers=args.providers, custom_prompt=args.prompt
        )
